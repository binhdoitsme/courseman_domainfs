...
package domainapp.modules.webserver.wsocket3way;
...
import domainapp.modules.webserver.wsocket.MessageBoardEndpoint.UppercaseEncoder;
...
public class Main {
  ...
  static Routing createRouting() {
    List<Class<? extends Encoder>> encoders = Collections
        .singletonList(UppercaseEncoder.class);

    return Routing.builder()
        .register("/rest", new TaskServer())        
        .register("/websocket",
            TyrusSupport.builder()
                .register(ServerEndpointConfig.Builder
                    .create(TaskServer.class, "/board")
                    .encoders(encoders).build())
                .build())
        .register("/web", StaticContentSupport.builder("/WEB3").build())
        .build();
  }
